...
digitMesh.character<-function(sdir, fixed, idxFixed = 1:fixed, GrOpt=setGraphicOptions(),
    ...
    TeOpt<-setTemplOptions(fixed, template=TeOpt$template, idxTemplate=TeOpt$idxTemplate,
    ...
    for (i in idxMesh){
        ...
        if (is.character(saveTPS)){
            save.tps(A[,,i], ID[i], saveTPS, sdir=full.dir, app=ifelse(cpt==1,append,TRUE),
...
#' @export
#'
setFileOptions<-function(M, sdir=getwd(), patt=".ply", deci.suffix=NULL, deci.dir="DecimMesh",
                         saveTPS="coord", overwrite=FALSE , append=FALSE){
    ...
    if (dir.exists(M)){
        ...
        if (file.exists(M)){
            ...
            stop("Providing filename doesn't exist...")
        }
    ...
    return(list(sdir=sdir, patt=patt, deci.suffix=deci.suffix, deci.dir=deci.dir,
                full.dir=full.dir, full.files=full.files, saveTPS=saveTPS, overwrite=overwrite ,
                append=append))
...
#'
setDecimOptions<-function(makeDecimation=TRUE, sequential=TRUE, tarface=15000, patt=".ply"){
    ...
    return(list(makeDecimation=makeDecimation, sequential=sequential, tarface=tarface, patt=patt))
