...
PlacePt <- function(x, y, verts, norms, mesh, start){
    ...
    if (length(selecMesh$vb) == 0 || !is.matrix(selecMesh$it) || length(int$vb) == 0 ||
        attr(try(meshintegrity(selecMesh, facecheck = TRUE), silent = TRUE), "class") == "try-error" ) {
...
SetPtZoom <- function(specFull, Pt, IdxPts = NULL, orthoplanes, idxPlanes,
    ...
    vd <- matrix(unlist(vd), nrow = length(tmp), ncol = 2, byrow = TRUE)
